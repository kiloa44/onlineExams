...
@section('content')
    ...
    <section id="sizing">
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">طلبات استبدال الطلبة</h4>
                                        ...
                                        <div class="col-sm-12">
                                            <div class="table-responsive">
                                                <table class="table table-bordered table-xs">
                                                    ...
                                                    <tbody>
                                                    @foreach($studentReplacements as $index => $studentReplacement)
                                                    <tr>
                                                        <th scope="col">{{ $index+1 }}</th>
                                                        <td>{{ $studentReplacement->date }}</td>
                                                        <td>{{ $studentReplacement->oldStudent->name }}</td>
                                                        <td>{{ $studentReplacement->newStudent->name }}</td>
                                                        <td>{{ $studentReplacement->status }}</td>
                                                        <td>
                                                            <!-- <div class="btn-group" role="group" aria-label="First Group">-->
                                                            <button type="button" class="btn btn-icon btn-light btn-sm" data-id="{{ $studentReplacement->id }}" onclick="editReplacementStudent(this)"><i class="fa fa-edit"></i></button>
                                                            <button type="button" class="btn btn-icon btn-danger btn-sm" data-id="{{ $studentReplacement->id }}" id="confirm-text" onclick="deleteReplacementStudent(this)" ><i class="fa fa-trash"></i></button>
            ...
            <div class="modal-dialog modal-xl" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <label class="modal-title text-text-bold-600" id="myModalLabel33">طالب استبدال جديد</label>
                    ...
                    <form id="form-addNewReplacmentStudent" action="#">
                            ...
                            <div class="form-body" style="padding:30px;">
                                ...
                                <div class="row">
                                    <div class="col-sm-12 text-center">
                                        <h3><u>نموذج استبدال طالب</u></h3>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-sm-12" style="background-color: #cccccc;">
                                        <h3><u>بيانات الطالب القديم</u></h3>
                                    </div>
                                </div>
                                <div class="form-group row mt-2">

                                    <label class="col-md-2 label-control" for="old_student_id" style="line-height: 40px;">الاسم رباعياً :</label>
                                    <div class="col-sm-6">
                                        <select id="old_student_id" class="form-control select2" name="old_student_id" onchange="getStudentInfo(this)">
                                ...
                                <div class="row">
                                    <div class="col-sm-12" style="background-color: #cccccc;">
                                        <h3><u>بيانات الطالب الجديد</u></h3>
                                    </div>
                                </div>
                                <div class="form-group row mt-2">

                                    <label class="col-md-2 label-control" for="new_student_id" style="line-height: 40px;">الاسم رباعياً : </label>
                                    <div class="col-sm-6">
                                        <select id="new_student_id" class="form-control select2" name="new_student_id" onchange="getStudentInfo(this)">
                                            <option value="null">-- اختر الطالب --</option>
                                            @foreach($students as $index => $student)
                                                <option value="{{ $student->id }}">{{ $student->name }}</option>
                                            @endforeach
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-primary" onclick="addNewReplacementStudent()" ><i class="fa fa-save"></i> اعتماد</button>
        ...
        <div class="modal fade text-left" id="editStudent" tabindex="-1" role="dialog" aria-labelledby="myModalLabel33"
            ...
            <div class="modal-dialog modal-xl" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <label class="modal-title text-text-bold-600" id="myModalLabel33">تعديل بيانات استبدال طالب</label>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <form id="form-editReplacmentStudent" action="#">
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-primary" ><i class="fa fa-save" onclick="saveReplacementStudent()"></i> حفظ</button>
                            <input type="reset" class="btn btn-secondary" data-dismiss="modal"
                                   value="إغلاق">
                        </div>
                    </form>
    ...
    <script>
        ...
        function getStudentInfo(item) {
            ...
            axios({
                method:'GET',
                url:'{{ route("showStudent") }}'+'/'+item.value,
                responseType:'json',
        ...
        function addNewReplacementStudent() {

            var data=getFormData($("#form-addNewReplacmentStudent"));
            //console.log(data);
            axios({
                method:'post',
                url:'{{ route("addNewStudentReplacement") }}',
                responseType:'json',
                data:data
            }).then(function (response) {
                success();
        ...
        function editReplacementStudent(item) {
            var id=$(item).attr('data-id');
            //console.log(id);
            axios({
                method:'GET',
                url:'{{ route("showStudentReplacement") }}'+'/'+id,
                responseType:'json',
            }).then(function (response) {
                console.log(response.data);
                $("#editStudent").modal('show');
                $('#editStudent #form-editReplacmentStudent #old_student_id').val(response.data.data.old_student_id).trigger('change');
                $('#editStudent #form-editReplacmentStudent #new_student_id').val(response.data.data.new_student_id).trigger('change');
        ...
        function saveReplacementStudent() {
            var data=getFormData($("#form-editReplacmentStudent"));
            axios({
                method:'PUT',
                url:'{{ route("updateStudentReplacement") }}'+'/'+data.id,
                responseType:'json',
                data:data,
                //params:{id}
            }).then(function (response) {
        ...
        function deleteReplacementStudent(item) {
            var id=$(item).attr('data-id');

            axios({
                method:'DELETE',
                url:'{{ route("deleteStudentReplacement") }}'+'/'+id,
                responseType:'json',
            }).then(function (response) {
                console.log(response.data);
            }).catch(function (error) {
